...
people_pd = pd.read_csv('./data/people.csv')
people = people_pd['itemLabel'].tolist()
...
with open(path + '/people_hyper.txt', 'a', encoding='utf-8') as f:
    for line in res:
        f.write(line + '\n')

list_hypon = hypon(people)
with open(path + '/people_hypon.txt', 'a', encoding='utf-8') as f:
...
compare_1 = compare(pattern_comp_1, people, 'богаче', 'беднее', 'состояние')
list_comp_big_1 = compare_1[0]
list_comp_small_1 = compare_1[1]
list_comp_attr_1 = compare_1[2]

compare_2 = compare(pattern_comp_2, people, 'богаче', 'беднее', 'состояние')
list_comp_big_2 = compare_2[0]
list_comp_small_2 = compare_2[1]
list_comp_attr_2 = compare_2[2]

compare_3 = compare(pattern_comp_3, people, 'богаче', 'беднее', 'состояние')
compare_4 = compare(pattern_comp_4, people, 'богаче', 'беднее', 'состояние')
...
with open(path + '/people_comp_attrmasked.txt', 'a', encoding='utf-8') as f:
    for line in res_2:
        f.write(line + '\n')

res_3 = random.sample(list_comp_attr_1, 8) + random.sample(list_comp_attr_2, 8) + random.sample(list_comp_attr_quest, 8)
with open(path + '/people_comp_valmasked.txt', 'a', encoding='utf-8') as f:
    for line in res_3:
        f.write(line + '\n')
...
single = single(pattern_single_big, pattern_single_small, people, 'состояние', 'богатый', 'бедный')
...
with open(path + '/people_single_attrmasked.txt', 'a', encoding='utf-8') as f:
    for line in res_4:
        f.write(line + '\n')

res_5 = random.sample(list_single_big_attr, 10) + random.sample(list_single_small_attr, 10)
with open(path + '/people_single_valmasked.txt', 'a', encoding='utf-8') as f:
    for line in res_5:
        f.write(line + '\n')
